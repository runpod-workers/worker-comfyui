...
def cleanup_lora_files(file_paths):
    ...
    for file_path in file_paths:
        try:
            # Only delete files that are in the download directory and have "pytorch_lora_weights.safetensors" in the filename
            if (os.path.exists(file_path) and 
                file_path.startswith(LORA_DIR) and 
                "pytorch_lora_weights.safetensors" in file_path):
                ...
                # Skip files that don't meet the criteria
                if not os.path.exists(file_path):
                    reason = "non-existent file"
                elif not file_path.startswith(LORA_DIR):
                    reason = "local file (not in download directory)"
                else:
                    reason = "filename does not contain 'pytorch_lora_weights.safetensors'"
    ...
    return {
        "status": "success",
        "message": "All matching lora files cleaned up successfully",

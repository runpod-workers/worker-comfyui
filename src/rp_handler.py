...
def process_output_images(outputs, job_id):
    ...
    to an AWS S3 bucket or as base64 encoded strings, depending on the
    ...
    - If the image exists, it checks if the AWS S3 bucket is configured via the BUCKET_ENDPOINT_URL environment variable.
    - If AWS S3 is configured, it uploads the image to the bucket and adds the URL to the results.
    - If AWS S3 is not configured, it encodes the image in base64 and adds the string to the results.
